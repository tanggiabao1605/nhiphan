...
export function decimalToBinary(num, length) {
  if (num < 0) {
      num = (1 << length) + num; // Chuyển đổi thành bù 2
  }
...
export function boothMultiplication(multiplicand, multiplier, bitLength) {
  ...
  let Q = decimalToBinary(multiplier, bitLength);
  let Q_1 = '0';
  let M = decimalToBinary(multiplicand, bitLength);
  ...
  let result = A + Q;
  let decimalResult = binaryToDecimal(result, true);
...
// Hàm chia hai số nhị phân (có thể có dấu)
// Hàm chia hai số nhị phân (có thể có dấu)
